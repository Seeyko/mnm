...
type MnMSprite = (typeof MNM_SPRITES)[number];
...
function spriteSize(sprite: MnMSprite): { width: number; height: number } {
  let width = 0;
...
export function AsciiArtAnimation() {
  ...
  useEffect(() => {
    ...
    function drawStaticFrame() {
      ...
      for (let baseRow = 1; baseRow < rows - 9; baseRow += gapY) {
        ...
        for (let baseCol = startX; baseCol < cols - 10; baseCol += gapX) {
          const sprite = MNM_SPRITES[(baseCol + baseRow) % MNM_SPRITES.length]!;
    ...
    function spawnClip() {
      const sprite = MNM_SPRITES[Math.floor(Math.random() * MNM_SPRITES.length)]!;
