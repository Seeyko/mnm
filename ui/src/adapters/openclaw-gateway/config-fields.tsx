...
export function OpenClawGatewayConfigFields({
  ...
  return (
    <>
      ...
      {!isCreate && (
        <>
          <Field label="MnM API URL override">
            <DraftInput
              value={
                eff(
                  "adapterConfig",
                  "mnmApiUrl",
                  String(config.mnmApiUrl ?? ""),
                )
              }
              onCommit={(v) => mark("adapterConfig", "mnmApiUrl", v || undefined)}
              immediate
              className={inputClass}
              placeholder="https://mnm.example"
            />
          </Field>
          ...
          {sessionStrategy === "fixed" && (
            <Field label="Session key">
              <DraftInput
                value={eff("adapterConfig", "sessionKey", String(config.sessionKey ?? "mnm"))}
          ...
          <Field label="Device auth">
            <div className="text-xs text-muted-foreground leading-relaxed">
              Always enabled for gateway agents. MnM persists a device key during onboarding so pairing approvals
