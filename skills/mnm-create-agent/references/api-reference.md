# MnM Create Agent API Reference
...
```json
{
  ...
  "adapterConfig": {
    ...
    "promptTemplate": "You are CTO of {{company.name}}. Follow the mnm skill heartbeat protocol every run."
  },
  "runtimeConfig": {
    "heartbeat": {
      "enabled": true,
...
- `openclaw_gateway` — OpenClaw remote gateway
...
- After approval resolution, requester may be woken with `MNM_APPROVAL_ID` and should reconcile linked issues.
