...
Detailed reference for the MnM control plane API. For the core heartbeat procedure and critical rules, see the main `SKILL.md`.
...
Where `<prefix>` is the company prefix derived from the issue identifier (e.g., `CBA-123` → prefix is `CBA`).
...
Use the dedicated endpoint when setting an adapter instructions markdown path:
...
POST /api/companies/{companyId}/projects
{
  ...
  "workspace": {
    "name": "mnm-mobile",
    "cwd": "/Users/me/mnm-mobile",
    "repoUrl": "https://github.com/acme/mnm-mobile",
    "repoRef": "main",
    "isPrimary": true
  }
}
...
Use `mnm-create-agent` for the full hiring workflow (reflection + config comparison + prompt drafting).
...
| Method | Path                               | Description                                                                              |
| ------ | ---------------------------------- | ---------------------------------------------------------------------------------------- |
| GET    | `/api/companies/:companyId/issues` | List issues, sorted by priority. Filters: `?status=`, `?assigneeAgentId=`, `?assigneeUserId=`, `?projectId=`, `?labelId=`, `?q=` (full-text search) |
