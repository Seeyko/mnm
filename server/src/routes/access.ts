...
function readSkillMarkdown(skillName: string): string | null {
  ...
  if (normalized !== "mnm" && normalized !== "mnm-create-agent")
    return null;
...
export function buildJoinDefaultsPayloadForAccept(input: {
  ...
  if (!nonEmptyTrimmedString(merged.mnmApiUrl)) {
    const legacyMnMApiUrl = nonEmptyTrimmedString(input.mnmApiUrl);
    if (legacyMnMApiUrl) merged.mnmApiUrl = legacyMnMApiUrl;
  }
...
function summarizeOpenClawGatewayDefaultsForLog(defaultsPayload: unknown) {
  ...
  return {
    ...
    mnmApiUrl: defaults
      ? nonEmptyTrimmedString(defaults.mnmApiUrl)
      : null,
...
export function normalizeAgentDefaultsForJoin(input: {
  ...
  const rawMnMApiUrl =
    typeof defaults.mnmApiUrl === "string"
      ? defaults.mnmApiUrl.trim()
      : "";
  if (rawMnMApiUrl) {
    try {
      const parsedMnMApiUrl = new URL(rawMnMApiUrl);
      if (
        parsedMnMApiUrl.protocol !== "http:" &&
        parsedMnMApiUrl.protocol !== "https:"
      ) {
        diagnostics.push({
          code: "openclaw_gateway_mnm_api_url_protocol",
          level: "warn",
          message: `mnmApiUrl must use http:// or https:// (got ${parsedMnMApiUrl.protocol}).`
        });
      } else {
        normalized.mnmApiUrl = parsedMnMApiUrl.toString();
        diagnostics.push({
          code: "openclaw_gateway_mnm_api_url_configured",
          level: "info",
          message: `mnmApiUrl set to ${parsedMnMApiUrl.toString()}`
        });
      }
    } catch {
      diagnostics.push({
        code: "openclaw_gateway_mnm_api_url_invalid",
        level: "warn",
        message: `Invalid mnmApiUrl: ${rawMnMApiUrl}`
      });
    }
  }
...
function buildOnboardingDiscoveryDiagnostics(input: {
  ...
  if (apiHost && isLoopbackHost(apiHost)) {
    diagnostics.push({
      ...
      level: "warn",
      message:
        "Onboarding URL resolves to loopback hostname. Remote OpenClaw agents cannot reach localhost on your MnM host.",
...
function buildInviteOnboardingManifest(
  ...
  const baseUrl = requestBaseUrl(req);
  const skillPath = "/api/skills/mnm";
  ...
  return {
    ...
    onboarding: {
      instructions:
        "Join as an OpenClaw Gateway agent, save your one-time claim secret, wait for board approval, then claim your API key. Save the claim response token to ~/.openclaw/workspace/mnm-claimed-api-key.json and load MNM_API_KEY from that file before starting heartbeat loops. You MUST submit adapterType='openclaw_gateway', set agentDefaultsPayload.url to your ws:// or wss:// OpenClaw gateway endpoint, and include agentDefaultsPayload.headers.x-openclaw-token (or legacy x-openclaw-auth).",
      ...
      requiredFields: {
        ...
        agentDefaultsPayload:
          "Adapter config for OpenClaw gateway. MUST include url (ws:// or wss://) and headers.x-openclaw-token (or legacy x-openclaw-auth). Optional fields: mnmApiUrl, waitTimeoutMs, sessionKeyStrategy, sessionKey, role, scopes, disableDeviceAuth, devicePrivateKeyPem."
      },
      ...
      skill: {
        ...
        path: skillPath,
        url: skillUrl,
        installPath: "~/.openclaw/skills/mnm/SKILL.md"
      }
    }
  };
}
...
export function buildInviteOnboardingTextDocument(
  ...
  appendBlock(`
    # MnM OpenClaw Gateway Onboarding
  ...
  appendBlock(`
    ...
    BODY="$(node -e '
      ...
      const body = {
        ...
        agentDefaultsPayload: {
          ...
          mnmApiUrl: "http://host.docker.internal:3100",
    ...
    - Keep device auth enabled (recommended). If devicePrivateKeyPem is omitted, MnM generates and persists one during join so pairing approvals are stable.
    ...
    Body (JSON):
    {
      ...
      "agentDefaultsPayload": {
        ...
        "mnmApiUrl": "https://mnm-hostname-your-agent-can-reach:3100",
    ...
    The board approves the join request in MnM before key claim is allowed.
    ...
    - ~/.openclaw/workspace/mnm-claimed-api-key.json
    chmod 600 ~/.openclaw/workspace/mnm-claimed-api-key.json

    And set the MNM_API_KEY and MNM_API_URL in your environment variables as specified here:
    ...
    Then set MNM_API_KEY and MNM_API_URL from the saved token field for every heartbeat run.
    ...
    ## Step 4: Install MnM skill in OpenClaw
    GET ${onboarding.skill.url}
    Install path: ${onboarding.skill.installPath}

    Be sure to prepend your MNM_API_URL to the top of your skill and note the path to your MNM_API_URL
    ...
    ${
      ...
      "Ensure MnM is reachable from your OpenClaw runtime."
    }
  `);
  ...
  if (connectionCandidates.length > 0) {
    lines.push("## Suggested MnM base URLs to try");
    ...
    appendBlock(`
      ...
      - set the first reachable candidate as agentDefaultsPayload.mnmApiUrl when submitting your join request
...
async function resolveActorEmail(db: Db, req: Request): Promise<string | null> {
  if (isLocalImplicit(req)) return "local@mnm.local";
...
export function accessRoutes(
  ...
  router.get("/skills/index", (_req, res) => {
    res.json({
      skills: [
        { name: "mnm", path: "/api/skills/mnm" },
        {
          name: "mnm-create-agent",
          path: "/api/skills/mnm-create-agent"
        }
      ]
    });
  });
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      const gatewayDefaultsPayload =
        requestType === "agent"
          ? buildJoinDefaultsPayloadForAccept({
              ...
              mnmApiUrl: req.body.mnmApiUrl ?? null,
      ...
      if (requestType === "agent" && adapterType === "openclaw_gateway") {
        ...
        if (
          expectedDefaults.mnmApiUrl &&
          !persistedDefaults.mnmApiUrl
        ) {
          missingPersistedFields.push("mnmApiUrl");
        }
