...
import { resolveMnMConfigPath, resolveMnMEnvPath } from "./paths.js";
...
function resolveAgentJwtSecretStatus(
  ...
  const envValue = process.env.MNM_AGENT_JWT_SECRET?.trim();
  if (envValue) {
    return {
      status: "pass",
      message: "set",
  ...
  if (existsSync(envFilePath)) {
    ...
    const fileValue = typeof parsed.MNM_AGENT_JWT_SECRET === "string" ? parsed.MNM_AGENT_JWT_SECRET.trim() : "";
    if (fileValue) {
      return {
        status: "warn",
...
export function printStartupBanner(opts: StartupBannerOptions): void {
  ...
  const art = [
    ...
    color("██║ ╚═╝ ██║ ██║  ██║ ██║ ╚═╝ ██║", "cyan"),
    color("╚═╝     ╚═╝ ╚═╝  ╚═╝ ╚═╝     ╚═╝", "cyan"),
    `    ${color("Make", "bold")}        ${color("No", "bold")}       ${color("Mistake", "bold")}`,
    `       ${color("Made By Studio Manifeste", "dim")}`,
  ];
