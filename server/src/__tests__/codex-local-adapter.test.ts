...
import { isCodexUnknownSessionError, parseCodexJsonl } from "@mnm/adapter-codex-local/server";
import { parseCodexStdoutLine } from "@mnm/adapter-codex-local/ui";
import { printCodexStreamEvent } from "@mnm/adapter-codex-local/cli";
  ...
  it("parses turn and reasoning lifecycle events", () => {
    ...
    expect(
      parseCodexStdoutLine(
        JSON.stringify({
          type: "item.completed",
          item: { id: "item_1", type: "reasoning", text: "**Preparing to use mnm skill**" },
        }),
        ts,
      ),
    ).toEqual([
      { kind: "thinking", ts, text: "**Preparing to use mnm skill**" },
    ]);
  });
