...
import { ensureCursorSkillsInjected } from "@mnm/adapter-cursor-local/server";
...
describe("cursor local adapter skill injection", () => {
  ...
  it("links missing MnM skills into Cursor skills home", async () => {
    const skillsDir = await makeTempDir("mnm-cursor-skills-src-");
    const skillsHome = await makeTempDir("mnm-cursor-skills-home-");
    cleanupDirs.add(skillsDir);
    cleanupDirs.add(skillsHome);

    await createSkillDir(skillsDir, "mnm");
    await createSkillDir(skillsDir, "mnm-create-agent");
    ...
    const injectedA = path.join(skillsHome, "mnm");
    const injectedB = path.join(skillsHome, "mnm-create-agent");
    expect((await fs.lstat(injectedA)).isSymbolicLink()).toBe(true);
    expect((await fs.lstat(injectedB)).isSymbolicLink()).toBe(true);
    expect(await fs.realpath(injectedA)).toBe(await fs.realpath(path.join(skillsDir, "mnm")));
    expect(await fs.realpath(injectedB)).toBe(
      await fs.realpath(path.join(skillsDir, "mnm-create-agent")),
    );
    expect(logs.some((line) => line.includes('Injected Cursor skill "mnm"'))).toBe(true);
    expect(logs.some((line) => line.includes('Injected Cursor skill "mnm-create-agent"'))).toBe(true);
  });

  it("preserves existing targets and only links missing skills", async () => {
    const skillsDir = await makeTempDir("mnm-cursor-preserve-src-");
    const skillsHome = await makeTempDir("mnm-cursor-preserve-home-");
    cleanupDirs.add(skillsDir);
    cleanupDirs.add(skillsHome);
    ...
    const existingTarget = path.join(skillsHome, "mnm");
    ...
    expect((await fs.lstat(path.join(skillsHome, "mnm-create-agent"))).isSymbolicLink()).toBe(true);
  });

  it("logs per-skill link failures and continues without throwing", async () => {
    const skillsDir = await makeTempDir("mnm-cursor-fail-src-");
    const skillsHome = await makeTempDir("mnm-cursor-fail-home-");
    cleanupDirs.add(skillsDir);
    cleanupDirs.add(skillsHome);
