...
import { execute, testEnvironment } from "@mnm/adapter-openclaw-gateway/server";
import { parseOpenClawGatewayStdoutLine } from "@mnm/adapter-openclaw-gateway/ui";
import type { AdapterExecutionContext } from "@mnm/adapter-utils";
...
describe("openclaw gateway adapter execute", () => {
  it("runs connect -> agent -> agent.wait and forwards wake payload", async () => {
    ...
    try {
      ...
      expect(payload?.sessionKey).toBe("mnm:issue:issue-123");
      expect(String(payload?.message ?? "")).toContain("wake now");
      expect(String(payload?.message ?? "")).toContain("MNM_RUN_ID=run-123");
      expect(String(payload?.message ?? "")).toContain("MNM_TASK_ID=task-123");
