...
describe("buildInviteOnboardingTextDocument", () => {
  it("renders a plain-text onboarding doc with expected endpoint references", () => {
    ...
    expect(text).toContain("MnM OpenClaw Gateway Onboarding");
    ...
    expect(text).toContain("Suggested MnM base URLs to try");
    expect(text).toContain("http://localhost:3100");
    expect(text).toContain("host.docker.internal");
    expect(text).toContain("mnmApiUrl");
    ...
    expect(text).toContain("set the first reachable candidate as agentDefaultsPayload.mnmApiUrl");
    expect(text).toContain("~/.openclaw/workspace/mnm-claimed-api-key.json");
    expect(text).toContain("MNM_API_KEY");
