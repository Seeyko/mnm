...
import type { MnMConfig } from "../config/schema.js";
...
import { printMnMCliBanner } from "../utils/banner.js";
...
type OnboardDefaults = Pick<MnMConfig, "database" | "logging" | "server" | "auth" | "storage" | "secrets">;

const ONBOARD_ENV_KEYS = [
  "MNM_PUBLIC_URL",
  "DATABASE_URL",
  "MNM_DB_BACKUP_ENABLED",
  "MNM_DB_BACKUP_INTERVAL_MINUTES",
  "MNM_DB_BACKUP_RETENTION_DAYS",
  "MNM_DB_BACKUP_DIR",
  "MNM_DEPLOYMENT_MODE",
  "MNM_DEPLOYMENT_EXPOSURE",
  "HOST",
  "PORT",
  "SERVE_UI",
  "MNM_ALLOWED_HOSTNAMES",
  "MNM_AUTH_BASE_URL_MODE",
  "MNM_AUTH_PUBLIC_BASE_URL",
  "BETTER_AUTH_URL",
  "BETTER_AUTH_BASE_URL",
  "MNM_STORAGE_PROVIDER",
  "MNM_STORAGE_LOCAL_DIR",
  "MNM_STORAGE_S3_BUCKET",
  "MNM_STORAGE_S3_REGION",
  "MNM_STORAGE_S3_ENDPOINT",
  "MNM_STORAGE_S3_PREFIX",
  "MNM_STORAGE_S3_FORCE_PATH_STYLE",
  "MNM_SECRETS_PROVIDER",
  "MNM_SECRETS_STRICT_MODE",
...
function quickstartDefaultsFromEnv(): {
  ...
  const instanceId = resolveMnMInstanceId();
  const defaultStorage = defaultStorageConfig();
  const defaultSecrets = defaultSecretsConfig();
  const databaseUrl = process.env.DATABASE_URL?.trim() || undefined;
  const publicUrl =
    process.env.MNM_PUBLIC_URL?.trim() ||
    process.env.MNM_AUTH_PUBLIC_BASE_URL?.trim() ||
    process.env.BETTER_AUTH_URL?.trim() ||
    process.env.BETTER_AUTH_BASE_URL?.trim() ||
    undefined;
  const deploymentMode =
    parseEnumFromEnv<DeploymentMode>(process.env.MNM_DEPLOYMENT_MODE, DEPLOYMENT_MODES) ?? "local_trusted";
  const deploymentExposureFromEnv = parseEnumFromEnv<DeploymentExposure>(
    process.env.MNM_DEPLOYMENT_EXPOSURE,
    DEPLOYMENT_EXPOSURES,
  );
  ...
  const authBaseUrlModeFromEnv = parseEnumFromEnv<AuthBaseUrlMode>(
    process.env.MNM_AUTH_BASE_URL_MODE,
    AUTH_BASE_URL_MODES,
  );
  const authBaseUrlMode = authBaseUrlModeFromEnv ?? (authPublicBaseUrl ? "explicit" : "auto");
  const allowedHostnamesFromEnv = process.env.MNM_ALLOWED_HOSTNAMES
    ? process.env.MNM_ALLOWED_HOSTNAMES
      .split(",")
  ...
  const storageProvider =
    parseEnumFromEnv<StorageProvider>(process.env.MNM_STORAGE_PROVIDER, STORAGE_PROVIDERS) ??
    defaultStorage.provider;
  const secretsProvider =
    parseEnumFromEnv<SecretProvider>(process.env.MNM_SECRETS_PROVIDER, SECRET_PROVIDERS) ??
    defaultSecrets.provider;
  const databaseBackupEnabled = parseBooleanFromEnv(process.env.MNM_DB_BACKUP_ENABLED) ?? true;
  const databaseBackupIntervalMinutes = Math.max(
    1,
    parseNumberFromEnv(process.env.MNM_DB_BACKUP_INTERVAL_MINUTES) ?? 60,
  );
  const databaseBackupRetentionDays = Math.max(
    1,
    parseNumberFromEnv(process.env.MNM_DB_BACKUP_RETENTION_DAYS) ?? 30,
  );
  const defaults: OnboardDefaults = {
    database: {
      ...
      backup: {
        ...
        dir: resolvePathFromEnv(process.env.MNM_DB_BACKUP_DIR) ?? resolveDefaultBackupDir(instanceId),
      },
    },
    logging: {
      mode: "file",
    ...
    storage: {
      ...
      localDisk: {
        baseDir:
          resolvePathFromEnv(process.env.MNM_STORAGE_LOCAL_DIR) ?? defaultStorage.localDisk.baseDir,
      },
      s3: {
        bucket: process.env.MNM_STORAGE_S3_BUCKET ?? defaultStorage.s3.bucket,
        region: process.env.MNM_STORAGE_S3_REGION ?? defaultStorage.s3.region,
        endpoint: process.env.MNM_STORAGE_S3_ENDPOINT ?? defaultStorage.s3.endpoint,
        prefix: process.env.MNM_STORAGE_S3_PREFIX ?? defaultStorage.s3.prefix,
        forcePathStyle:
          parseBooleanFromEnv(process.env.MNM_STORAGE_S3_FORCE_PATH_STYLE) ??
          defaultStorage.s3.forcePathStyle,
      },
    },
    secrets: {
      provider: secretsProvider,
      strictMode: parseBooleanFromEnv(process.env.MNM_SECRETS_STRICT_MODE) ?? defaultSecrets.strictMode,
      localEncrypted: {
        keyFilePath:
          resolvePathFromEnv(process.env.MNM_SECRETS_MASTER_KEY_FILE) ??
          defaultSecrets.localEncrypted.keyFilePath,
      },
    },
  };
  const ignoredEnvKeys: Array<{ key: string; reason: string }> = [];
  if (deploymentMode === "local_trusted" && process.env.MNM_DEPLOYMENT_EXPOSURE !== undefined) {
    ignoredEnvKeys.push({
      key: "MNM_DEPLOYMENT_EXPOSURE",
...
export async function onboard(opts: OnboardOptions): Promise<void> {
  printMnMCliBanner();
  p.intro(pc.bgCyan(pc.black(" mnm onboard ")));
  const configPath = resolveConfigPath(opts.config);
  const instance = describeLocalInstancePaths(resolveMnMInstanceId());
  p.log.message(
    pc.dim(
  ...
  let llm: MnMConfig["llm"] | undefined;
  ...
  if (setupMode === "advanced") {
    ...
    if (database.mode === "postgres" && database.connectionString) {
      ...
      try {
        const { createDb } = await import("@mnm/db");
        const db = createDb(database.connectionString);
        await db.execute("SELECT 1");
        s.stop("Database connection successful");
      } catch {
        s.stop(pc.yellow("Could not connect to database — you can fix this later with `mnm doctor`"));
      }
    }
  ...
  if (jwtSecret.created) {
    p.log.success(`Created ${pc.cyan("MNM_AGENT_JWT_SECRET")} in ${pc.dim(envFilePath)}`);
  } else if (process.env.MNM_AGENT_JWT_SECRET?.trim()) {
    p.log.info(`Using existing ${pc.cyan("MNM_AGENT_JWT_SECRET")} from environment`);
  } else {
    p.log.info(`Using existing ${pc.cyan("MNM_AGENT_JWT_SECRET")} in ${pc.dim(envFilePath)}`);
  }

  const config: MnMConfig = {
    $meta: {
      version: 1,
  ...
  p.note(
    [
      ...
      "Agent auth: MNM_AGENT_JWT_SECRET configured",
    ].join("\n"),
    "Configuration saved",
  );
  ...
  p.note(
    [
      `Run: ${pc.cyan("mnm run")}`,
      `Reconfigure later: ${pc.cyan("mnm configure")}`,
      `Diagnose setup: ${pc.cyan("mnm doctor")}`,
    ].join("\n"),
    "Next commands",
  );
  ...
  if (shouldRunNow && !opts.invokedByRun) {
    process.env.MNM_OPEN_ON_LISTEN = "true";
