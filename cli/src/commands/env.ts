...
const DEFAULT_AGENT_JWT_ISSUER = "mnm";
const DEFAULT_AGENT_JWT_AUDIENCE = "mnm-api";
...
function defaultSecretsKeyFilePath(): string {
  return resolveDefaultSecretsKeyFilePath(resolveMnMInstanceId());
}
function defaultStorageBaseDir(): string {
  return resolveDefaultStorageDir(resolveMnMInstanceId());
}

export async function envCommand(opts: { config?: string }): Promise<void> {
  p.intro(pc.bgCyan(pc.black(" mnm env ")));

  const configPath = resolveConfigPath(opts.config);
  let config: MnMConfig | null = null;
...
function collectDeploymentEnvRows(config: MnMConfig | null, configPath: string): EnvVarRow[] {
  ...
  const publicUrl =
    ...
    process.env.MNM_AUTH_PUBLIC_BASE_URL ??
  ...
  const publicUrlSource: EnvSource =
    process.env.MNM_PUBLIC_URL
      ? "env"
      : process.env.MNM_AUTH_PUBLIC_BASE_URL || process.env.BETTER_AUTH_URL || process.env.BETTER_AUTH_BASE_URL
        ? "env"
        : config?.auth?.publicBaseUrl
          ? "config"
          : "missing";
  ...
  const rows: EnvVarRow[] = [
    ...
    },
    {
      ...
      source: process.env.BETTER_AUTH_TRUSTED_ORIGINS
        ...
        : trustedOriginsDefault
          ...
          : "missing",
      required: false,
      note: "Comma-separated auth origin allowlist (auto-derived from MNM_PUBLIC_URL when possible)",
    },
    {
      key: "MNM_AGENT_JWT_TTL_SECONDS",
      value: process.env.MNM_AGENT_JWT_TTL_SECONDS ?? DEFAULT_AGENT_JWT_TTL_SECONDS,
      source: process.env.MNM_AGENT_JWT_TTL_SECONDS ? "env" : "default",
      required: false,
      note: "JWT lifetime in seconds",
    },
    {
      key: "MNM_AGENT_JWT_ISSUER",
      value: process.env.MNM_AGENT_JWT_ISSUER ?? DEFAULT_AGENT_JWT_ISSUER,
      source: process.env.MNM_AGENT_JWT_ISSUER ? "env" : "default",
      required: false,
      note: "JWT issuer",
    },
    {
      key: "MNM_AGENT_JWT_AUDIENCE",
      value: process.env.MNM_AGENT_JWT_AUDIENCE ?? DEFAULT_AGENT_JWT_AUDIENCE,
      source: process.env.MNM_AGENT_JWT_AUDIENCE ? "env" : "default",
      required: false,
      note: "JWT audience",
    },
    {
    ...
    {
      key: "MNM_SECRETS_PROVIDER",
      value: secretsProvider,
      source: process.env.MNM_SECRETS_PROVIDER
        ? "env"
        : config?.secrets?.provider
          ? "config"
          : "default",
    ...
    {
      key: "MNM_SECRETS_STRICT_MODE",
      value: secretsStrictMode,
      source: process.env.MNM_SECRETS_STRICT_MODE
        ? "env"
    ...
    {
      key: "MNM_SECRETS_MASTER_KEY_FILE",
      value: secretsKeyFilePath,
      source: process.env.MNM_SECRETS_MASTER_KEY_FILE
        ? "env"
        : config?.secrets?.localEncrypted?.keyFilePath
          ? "config"
          : "default",
    ...
    {
      key: "MNM_STORAGE_PROVIDER",
      value: storageProvider,
      source: process.env.MNM_STORAGE_PROVIDER
        ? "env"
        : config?.storage?.provider
          ? "config"
          : "default",
    ...
    {
      key: "MNM_STORAGE_LOCAL_DIR",
      value: storageLocalDir,
      source: process.env.MNM_STORAGE_LOCAL_DIR
        ? "env"
        : config?.storage?.localDisk?.baseDir
          ? "config"
          : "default",
    ...
    {
      key: "MNM_STORAGE_S3_BUCKET",
      value: storageS3Bucket,
      source: process.env.MNM_STORAGE_S3_BUCKET
        ? "env"
        : config?.storage?.s3?.bucket
          ? "config"
          : "default",
    ...
    {
      key: "MNM_STORAGE_S3_REGION",
      value: storageS3Region,
      source: process.env.MNM_STORAGE_S3_REGION
        ? "env"
        : config?.storage?.s3?.region
          ? "config"
          : "default",
    ...
    {
      key: "MNM_STORAGE_S3_ENDPOINT",
      value: storageS3Endpoint,
      source: process.env.MNM_STORAGE_S3_ENDPOINT
        ? "env"
        : config?.storage?.s3?.endpoint
          ? "config"
          : "default",
    ...
    {
      key: "MNM_STORAGE_S3_PREFIX",
      value: storageS3Prefix,
      source: process.env.MNM_STORAGE_S3_PREFIX
        ? "env"
        : config?.storage?.s3?.prefix
          ? "config"
          : "default",
    ...
    {
      key: "MNM_STORAGE_S3_FORCE_PATH_STYLE",
      value: storageS3ForcePathStyle,
      source: process.env.MNM_STORAGE_S3_FORCE_PATH_STYLE
        ? "env"
  ...
  if (process.env.MNM_CONFIG || configPath !== defaultConfigPath) {
    rows.push({
      key: "MNM_CONFIG",
      value: process.env.MNM_CONFIG ?? configPath,
      source: process.env.MNM_CONFIG ? "env" : "default",
      required: false,
