...
import { ApiRequestError, MnMApiClient } from "../../client/http.js";
...
export function addCommonClientOptions(command: Command, opts?: { includeCompany?: boolean }): Command {
  command
    .option("-c, --config <path>", "Path to MnM config file")
    .option("-d, --data-dir <path>", "MnM data directory root (isolates state from ~/.mnm)")
    .option("--context <path>", "Path to CLI context file")
    .option("--profile <name>", "CLI context profile name")
    .option("--api-base <url>", "Base URL for the MnM API")
...
export function resolveCommandContext(
  ...
  const apiBase =
    ...
    process.env.MNM_API_URL?.trim() ||
    profile.apiBase ||
    inferApiBaseFromConfig(options.config);

  const apiKey =
    options.apiKey?.trim() ||
    process.env.MNM_API_KEY?.trim() ||
    readKeyFromProfileEnv(profile);

  const companyId =
    options.companyId?.trim() ||
    process.env.MNM_COMPANY_ID?.trim() ||
    profile.companyId;

  if (opts?.requireCompany && !companyId) {
    throw new Error(
      "Company ID is required. Pass --company-id, set MNM_COMPANY_ID, or set context profile companyId via `mnm context set`.",
    );
  }

  const api = new MnMApiClient({ apiBase, apiKey });
  return {
    api,
    companyId,
    profileName,
...
function inferApiBaseFromConfig(configPath?: string): string {
  const envHost = process.env.MNM_SERVER_HOST?.trim() || "localhost";
  let port = Number(process.env.MNM_SERVER_PORT || "");
