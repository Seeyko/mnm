...
import { formatDatabaseBackupResult, runDatabaseBackup } from "@mnm/db";
import {
  expandHomePrefix,
...
function resolveConnectionString(configPath?: string): { value: string; source: string } {
  ...
  return {
    value: `postgres://mnm:mnm@127.0.0.1:${port}/mnm`,
...
export async function dbBackupCommand(opts: DbBackupOptions): Promise<void> {
  printMnMCliBanner();
  p.intro(pc.bgCyan(pc.black(" mnm db:backup ")));

  const configPath = resolveConfigPath(opts.config);
  const config = readConfig(opts.config);
  const connection = resolveConnectionString(opts.config);
  const defaultDir = resolveDefaultBackupDir(resolveMnMInstanceId());
  ...
  const filenamePrefix = opts.filenamePrefix?.trim() || "mnm";
