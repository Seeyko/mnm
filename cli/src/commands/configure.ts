...
function defaultConfig(): MnMConfig {
...
export async function configure(opts: {
  ...
  printMnMCliBanner();
  p.intro(pc.bgCyan(pc.black(" mnm configure ")));
  const configPath = resolveConfigPath(opts.config);

  if (!configExists(opts.config)) {
    p.log.error("No config file found. Run `mnm onboard` first.");
    p.outro("");
    return;
  }

  let config: MnMConfig;
  try {
      ...
      case "secrets":
        config.secrets = await promptSecrets(config.secrets);
        {
          ...
          if (keyResult.status === "created") {
            ...
            p.log.message(pc.dim("Skipping local key file management because MNM_SECRETS_MASTER_KEY is set"));
          }
        }
        break;
    }
