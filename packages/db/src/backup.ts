...
function resolveMnMHomeDir(): string {
  const envHome = process.env.MNM_HOME?.trim();
  if (envHome) return path.resolve(expandHomePrefix(envHome));
  return path.resolve(os.homedir(), ".mnm");
}

function resolveMnMInstanceId(): string {
  const raw = process.env.MNM_INSTANCE_ID?.trim() || "default";
...
function resolveDefaultConfigPath(): string {
  return path.resolve(resolveMnMHomeDir(), "instances", resolveMnMInstanceId(), "config.json");
}
...
function resolveConnectionString(config: PartialConfig | null): string {
  ...
  return `postgres://mnm:mnm@127.0.0.1:${port}/mnm`;
}

function resolveDefaultBackupDir(): string {
  return path.resolve(resolveMnMHomeDir(), "instances", resolveMnMInstanceId(), "data", "backups");
}
