...
Use when:
- You want MnM to run Pi (the AI coding agent) locally as the agent runtime
...
- MnM requires an explicit \`model\` value for \`pi_local\` agents.
- Sessions are stored in ~/.pi/mnms/ and resumed with --session.
