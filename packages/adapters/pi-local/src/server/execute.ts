...
const MNM_SESSIONS_DIR = path.join(os.homedir(), ".pi", "mnms");
...
async function ensurePiSkillsInjected(onLog: AdapterExecutionContext["onLog"]) {
  ...
  for (const entry of entries) {
    ...
    try {
      ...
      await onLog(
        "stderr",
        `[mnm] Injected Pi skill "${entry.name}" into ${piSkillsHome}\n`,
      );
    } catch (err) {
      await onLog(
        "stderr",
        `[mnm] Failed to inject Pi skill "${entry.name}" into ${piSkillsHome}: ${err instanceof Error ? err.message : String(err)}\n`,
      );
    }
  }
}
...
async function ensureSessionsDir(): Promise<string> {
  await fs.mkdir(MNM_SESSIONS_DIR, { recursive: true });
  return MNM_SESSIONS_DIR;
}

function buildSessionPath(agentId: string, timestamp: string): string {
  const safeTimestamp = timestamp.replace(/[:.]/g, "-");
  return path.join(MNM_SESSIONS_DIR, `${safeTimestamp}-${agentId}.jsonl`);
}
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  if (wakeTaskId) env.MNM_TASK_ID = wakeTaskId;
  if (wakeReason) env.MNM_WAKE_REASON = wakeReason;
  if (wakeCommentId) env.MNM_WAKE_COMMENT_ID = wakeCommentId;
  if (approvalId) env.MNM_APPROVAL_ID = approvalId;
  if (approvalStatus) env.MNM_APPROVAL_STATUS = approvalStatus;
  if (linkedIssueIds.length > 0) env.MNM_LINKED_ISSUE_IDS = linkedIssueIds.join(",");
  if (workspaceCwd) env.MNM_WORKSPACE_CWD = workspaceCwd;
  if (workspaceSource) env.MNM_WORKSPACE_SOURCE = workspaceSource;
  if (workspaceId) env.MNM_WORKSPACE_ID = workspaceId;
  if (workspaceRepoUrl) env.MNM_WORKSPACE_REPO_URL = workspaceRepoUrl;
  if (workspaceRepoRef) env.MNM_WORKSPACE_REPO_REF = workspaceRepoRef;
  if (workspaceHints.length > 0) env.MNM_WORKSPACES_JSON = JSON.stringify(workspaceHints);
  ...
  if (runtimeSessionId && !canResumeSession) {
    await onLog(
      "stderr",
      `[mnm] Pi session "${runtimeSessionId}" was saved for cwd "${runtimeSessionCwd}" and will not be resumed in "${cwd}".\n`,
    );
  }
  ...
  if (resolvedInstructionsFilePath) {
    try {
      ...
      await onLog(
        "stderr",
        `[mnm] Loaded agent instructions file: ${resolvedInstructionsFilePath}\n`,
      );
    } catch (err) {
      ...
      await onLog(
        "stderr",
        `[mnm] Warning: could not read agent instructions file "${resolvedInstructionsFilePath}": ${reason}\n`,
      );
  ...
  if (
    ...
    await onLog(
      "stderr",
      `[mnm] Pi session "${runtimeSessionId}" is unavailable; retrying with a fresh session.\n`,
    );
