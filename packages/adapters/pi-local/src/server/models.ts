...
import type { AdapterModel } from "@mnm/adapter-utils";
import { asString, runChildProcess } from "@mnm/adapter-utils/server-utils";
...
function resolvePiCommand(input: unknown): string {
  const envOverride =
    typeof process.env.MNM_PI_COMMAND === "string" &&
    process.env.MNM_PI_COMMAND.trim().length > 0
      ? process.env.MNM_PI_COMMAND.trim()
      : "pi";
  return asString(input, envOverride);
}

const discoveryCache = new Map<string, { expiresAt: number; models: AdapterModel[] }>();
const VOLATILE_ENV_KEY_PREFIXES = ["MNM_", "npm_", "NPM_"] as const;
