...
function renderMnMEnvNote(env: Record<string, string>): string {
  const mnmKeys = Object.keys(env)
    .filter((key) => key.startsWith("MNM_"))
    .sort();
  if (mnmKeys.length === 0) return "";
  return [
    "MnM runtime note:",
    `The following MNM_* environment variables are available in this run: ${mnmKeys.join(", ")}`,
...
export async function ensureCursorSkillsInjected(
  ...
  const skillsDir = options.skillsDir ?? await resolveMnMSkillsDir();
  if (!skillsDir) return;
  ...
  try {
    ...
    await onLog(
      "stderr",
      `[mnm] Failed to prepare Cursor skills directory ${skillsHome}: ${err instanceof Error ? err.message : String(err)}\n`,
    );
    return;
  }
  ...
  try {
    ...
    await onLog(
      "stderr",
      `[mnm] Failed to read MnM skills from ${skillsDir}: ${err instanceof Error ? err.message : String(err)}\n`,
    );
    return;
  }
  ...
  for (const entry of entries) {
    ...
    try {
      ...
      await onLog(
        "stderr",
        `[mnm] Injected Cursor skill "${entry.name}" into ${skillsHome}\n`,
      );
    } catch (err) {
      await onLog(
        "stderr",
        `[mnm] Failed to inject Cursor skill "${entry.name}" into ${skillsHome}: ${err instanceof Error ? err.message : String(err)}\n`,
      );
    }
  }
}
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  if (runtimeSessionId && !canResumeSession) {
    await onLog(
      "stderr",
      `[mnm] Cursor session "${runtimeSessionId}" was saved for cwd "${runtimeSessionCwd}" and will not be resumed in "${cwd}".\n`,
    );
  }
  ...
  if (instructionsFilePath) {
    try {
      ...
      await onLog(
        "stderr",
        `[mnm] Loaded agent instructions file: ${instructionsFilePath}\n`,
      );
    } catch (err) {
      const reason = err instanceof Error ? err.message : String(err);
      await onLog(
        "stderr",
        `[mnm] Warning: could not read agent instructions file "${instructionsFilePath}": ${reason}\n`,
      );
    }
  }
  const commandNotes = (() => {
  ...
  const mnmEnvNote = renderMnMEnvNote(env);
  const prompt = `${instructionsPrefix}${mnmEnvNote}${renderedPrompt}`;
  ...
  if (
    ...
    await onLog(
      "stderr",
      `[mnm] Cursor resume session "${sessionId}" is unavailable; retrying with a fresh session.\n`,
    );
