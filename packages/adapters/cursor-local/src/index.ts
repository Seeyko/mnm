...
Use when:
- You want MnM to run Cursor Agent CLI locally as the agent runtime
...
- MnM auto-injects local skills into "~/.cursor/skills" when missing, so Cursor can discover "$mnm" and related skills on local runs.
- MnM auto-adds --yolo unless one of --trust/--yolo/-f is already present in extraArgs.
`;
