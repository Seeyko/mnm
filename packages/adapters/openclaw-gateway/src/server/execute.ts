...
import { asNumber, asString, buildMnMEnv, parseObject } from "@mnm/adapter-utils/server-utils";
...
const DEFAULT_CLIENT_VERSION = "mnm";
...
function resolveSessionKey(input: {
  ...
  const fallback = input.configuredSessionKey ?? "mnm";
  if (input.strategy === "run") return `mnm:run:${input.runId}`;
  if (input.strategy === "issue" && input.issueId) return `mnm:issue:${input.issueId}`;
  return fallback;
}
...
function resolveMnMApiUrlOverride(value: unknown): string | null {
...
function buildMnMEnvForWake(ctx: AdapterExecutionContext, wakePayload: WakePayload): Record<string, string> {
  const mnmApiUrlOverride = resolveMnMApiUrlOverride(ctx.config.mnmApiUrl);
  const mnmEnv: Record<string, string> = {
    ...buildMnMEnv(ctx.agent),
    MNM_RUN_ID: ctx.runId,
  };

  if (mnmApiUrlOverride) {
    mnmEnv.MNM_API_URL = mnmApiUrlOverride;
  }
  if (wakePayload.taskId) mnmEnv.MNM_TASK_ID = wakePayload.taskId;
  if (wakePayload.wakeReason) mnmEnv.MNM_WAKE_REASON = wakePayload.wakeReason;
  if (wakePayload.wakeCommentId) mnmEnv.MNM_WAKE_COMMENT_ID = wakePayload.wakeCommentId;
  if (wakePayload.approvalId) mnmEnv.MNM_APPROVAL_ID = wakePayload.approvalId;
  if (wakePayload.approvalStatus) mnmEnv.MNM_APPROVAL_STATUS = wakePayload.approvalStatus;
  if (wakePayload.issueIds.length > 0) {
    mnmEnv.MNM_LINKED_ISSUE_IDS = wakePayload.issueIds.join(",");
  }

  return mnmEnv;
}

function buildWakeText(payload: WakePayload, mnmEnv: Record<string, string>): string {
  const claimedApiKeyPath = "~/.openclaw/workspace/mnm-claimed-api-key.json";
  const orderedKeys = [
    "MNM_RUN_ID",
    "MNM_AGENT_ID",
    "MNM_COMPANY_ID",
    ...
    "MNM_APPROVAL_ID",
    "MNM_APPROVAL_STATUS",
    "MNM_LINKED_ISSUE_IDS",
  ];

  const envLines: string[] = [];
  for (const key of orderedKeys) {
    const value = mnmEnv[key];
    if (!value) continue;
    envLines.push(`${key}=${value}`);
  }

  const issueIdHint = payload.taskId ?? payload.issueId ?? "";
  const apiBaseHint = mnmEnv.MNM_API_URL ?? "<set MNM_API_URL>";

  const lines = [
    "MnM wake event for a cloud adapter.",
    "",
    "Run this procedure now. Do not guess undocumented endpoints and do not ask for additional heartbeat docs.",
    "",
    "Set these values in your run context:",
    ...envLines,
    `MNM_API_KEY=<token from ${claimedApiKeyPath}>`,
    "",
    `Load MNM_API_KEY from ${claimedApiKeyPath} (the token you saved after claim-api-key).`,
    "",
    ...
    "",
    "HTTP rules:",
    "- Use Authorization: Bearer $MNM_API_KEY on every API call.",
    "- Use X-MnM-Run-Id: $MNM_RUN_ID on every mutating API call.",
    "- Use only /api endpoints listed below.",
    "- Do NOT call guessed endpoints like /api/cloud-adapter/*, /api/cloud-adapters/*, /api/adapters/cloud/*, or /api/heartbeat.",
    "",
    "Workflow:",
    "1) GET /api/agents/me",
    `2) Determine issueId: MNM_TASK_ID if present, otherwise issue_id (${issueIdHint}).`,
    "3) If issueId exists:",
    "   - POST /api/issues/{issueId}/checkout with {\"agentId\":\"$MNM_AGENT_ID\",\"expectedStatuses\":[\"todo\",\"backlog\",\"blocked\"]}",
    ...
    "   - GET /api/companies/$MNM_COMPANY_ID/issues?assigneeAgentId=$MNM_AGENT_ID&status=todo,in_progress,blocked",
...
class GatewayWsClient {
  ...
  close() {
    if (!this.ws) return;
    this.ws.close(1000, "mnm-complete");
    this.ws = null;
  }
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const mnmEnv = buildMnMEnvForWake(ctx, wakePayload);
  const wakeText = buildWakeText(wakePayload, mnmEnv);
