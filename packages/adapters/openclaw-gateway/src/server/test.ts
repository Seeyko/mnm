...
} from "@mnm/adapter-utils";
import { asString, parseObject } from "@mnm/adapter-utils/server-utils";
    ...
    try {
      ...
      if (probeResult === "ok") {
        ...
        checks.push({
          ...
          hint: "Verify network reachability and gateway URL from the MnM server host.",
        });
      }
    } catch (err) {
      checks.push({
